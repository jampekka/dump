...
(function($) {
    ...
    $.quiz = function(quiz_data, results_data, options) {
		...
		var default_html = null;
		$.each(data, function(index, value) {
			if(value.numberofrightanswers === "") {
				default_html = value.html;
				return false;
			}
		});
		...
		for (var i = 0; i < data.length; i++) {
		    ...
		    args = {right_answers: right_answers,
		    	total_questions: self.quiz_data.length};
                    html = _.template(this.results_data[right_answers])(args);
                }
